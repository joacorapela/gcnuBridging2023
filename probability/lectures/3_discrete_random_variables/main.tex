...
\begin{equation*}
    \underset{X \sim f_X}{\mathbb{E}}\left[g\left(X\right)\right] \approx \frac{1}{N}\sum_{n = 1}^N g\left(x_n\right)\qquad\textrm{for large }N
\end{equation*}
\end{alertblock}}
\end{frame}
...
\begin{equation*}
\begin{aligned}
    \mathbb{P}\left(X_1 = x_1, \dots, X_N = x_N\right) &= \prod_{n = 1}^N\mathbb{P}\left(X_n = x_n\right)\\
    &= \prod_{n = 1}^N p^{x_n}\left(1 - p\right)^{1 - x_n}\\
...
\begin{itemize}[<+->]
    ...
    \item \(N\left(t + \tau\right) - N\left(t\right) \sim \textrm{Pois}\left(\tau\lambda\right)\) for all \(t \in T\),
    \item and \(N\left(\tau_2\right) - N\left(\tau_1\right)\) and \(N\left(\tau_4\right) - N\left(\tau_3\right)\) are independent for all disjoint intervals \(\left(\tau_1, \tau_2\right]\) and \(\left(\tau_3, \tau_4\right]\),
...
At any time \(t\), what is the waiting time until the next increment in \(N\left(t\right)\)? Find out in the next lecture.}
\end{frame}
