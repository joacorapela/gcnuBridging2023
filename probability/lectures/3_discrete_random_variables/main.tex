...
f_X\left(x\right) =
\begin{cases}
    \frac{1}{6} & \text{if } x \in \mathcal{X}\\
    0 & \text{otherwise}
\end{cases}\,.
\end{equation*}}
...
%     The support and PMF for each random variable are the same as in the last example. That is, \(X_1, \dots, X_N \overset{\text{i.i.d.}}{\sim} f_x\).
...
\begin{itemize}[<+->]
    \item \(\text{Var}\left(aX + b\right) = a^2\text{Var}\left(X\right)\) for constants \(a, b\).
    \item \(\text{Var}\left(a\right) = 0\) for constant \(a\).
    \item \(\text{Var}\left(X\right) \geq 0\).
\end{itemize}
